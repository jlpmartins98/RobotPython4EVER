...
from ev3dev2.motor import MoveTank, OUTPUT_A, OUTPUT_C,SpeedPercent, MoveDifferential, SpeedRPM
...
arrayCacifos_com_heuristica = [cacifo(None,1,0,11)] #array que guarda os cacifos com o seu numero e heuristica ate á cerca
...
def CacifoAtual(numero_cacifo):
    cacifoFiller = cacifo(None,100,200,300) #caso esteja a procura dum cacifo fora do tabuleiro
...
def algoritmo_A_star(goal):#devolve um array com o caminho(nº dos cacifos a seguir começando na sua posiçao)
    ...
    arrayBackup = cp.deepcopy(arrayCacifos_com_heuristica) #faz uma copia da cacifoComParedeiavel
    ...
    while(len(openList) > 0):
        ...
        if(currentNode.paredeUp == True or currentNode.numeroCacifo >= 31):#caso tenha uma parede em cima ou esteja nos cacifos do topo
...
def verifica_cacifo():
    ...
    if(cs.color == 5):  #Encontrou parede RED
        ...
        i +=1                   #Atualiza o i
        vira(90)
    elif(cs.color == 1): #Encontrou limite do cacifo BLACK
        ovelhas()
